...
tf.app.flags.DEFINE_integer('batch_size', 20, "Network batch size during training")
tf.app.flags.DEFINE_integer('max_iters', 50, "Max iteration")
tf.app.flags.DEFINE_integer('step_size', 30, "Step size for reducing the learning rate, currently only support one step")
...
tf.app.flags.DEFINE_integer('snapshot_iterations', 10, "Iteration to take snapshot")
...
tf.app.flags.DEFINE_float('roi_bg_threshold_low', 0.0, "Overlap threshold for a ROI to be considered background (class = 0 if overlap in [LO, HI))")#0.1
